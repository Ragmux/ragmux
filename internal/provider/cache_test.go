...
func TestAnthropicCacheControlPlacements(t *testing.T) {
	...
	ar, err := translateAnthropic(context.Background(), Config{Model: "claude"}, req)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnthropicSystemShapeUnchangedWithoutCacheControl(t *testing.T) {
	for _, body := range []string{
		...
		ar, err := translateAnthropic(context.Background(), Config{Model: "claude"}, userRequest(t, body))
		if err != nil {
			t.Fatal(err)
		}
