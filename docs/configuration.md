...
| `IMAGE_FETCH_MAX_CONCURRENT` | `16` | Image fetches the whole process runs at once (`>= 1`), and the most connections the image transport will hold to one host. `IMAGE_FETCH_MAX_PER_REQUEST` bounds a single request and nothing across them; this is what bounds the fan-out when many arrive together, so a key holder cannot turn the gateway into an amplifier pointed at a host of their choosing. **No single project (or, for a key that named none, key) may hold more than half of these slots**, so one tenant aimed at a slow image host cannot starve another. A fetch that cannot get a slot within two seconds gets `429` with `Retry-After` and `code: "image_fetch_saturated"` — a resource limit, not a fault, and deliberately not a `5xx` the official SDKs would retry automatically into the same full queue. The queue wait is separate from `IMAGE_FETCH_TIMEOUT`, which starts only once a slot is in hand. |
